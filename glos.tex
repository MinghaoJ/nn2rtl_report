...
\newacronym[%
    description={Directed Acyclic Graph, a graph with directed edges and contains no cycles.}%
...
\newacronym[%
    description={Block RAM (see RAM). A type of memory block in FPGA fabrics that can be linked together to create memoriy banks of various dimensions.}%
]{bram}{BRAM}{Block RAM}

\newacronym[%
    description={Dynamic RAM (see RAM). RAM that cannot retain data indefinitely and requires periodic ``refreshing,'' where the data contents are read and re-written.}%
]{dram}{DRAM}{Dynamic RAM}
\glssee{dram}{ram}
\glssee[Compare with:]{dram}{sram}

\newacronym[%
    description={Static RAM (see RAM). RAM that retains its data so long as it is powered (compare with DRAM).}%
