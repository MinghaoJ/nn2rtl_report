...
\usepackage[backend=biber,style=ieee,isbn=false,doi=false,url=false]{biblatex}
...
TensorFlow provides a toolset under TensorFlow Lite containing tools for quantizing a fully trained inference model. The final compressed model is converted back to a Protocol Buffer file and saved to the output path specified in the configuration file.
%The code snippet in Listing \ref{lst:optimize} shows the Google's TensorFlow Lite example provided for using graph transforms to quantize weights and strip unused nodes.
...
A series of multipliers perform the multiplication between each of the matrix entries, followed by a series of adders which compute the sum of these products. This operation is referred to as a \gls{mac}. The dimensions of the inputs and outputs are constrained within a layer, allowing the generation of \gls{rtl} modules by analyzing each layer.
...
Instead, each unique \gls{filter} receives dedicated hardware, and their matrix \gls{mac} operations are performed serially over the input volume. This results in a great reduction of the number of multipliers. Thus, each layer of the \gls{neural_net} would map to a dedicated bank of logic that would perform its sequence of multiplications.
...
\glspl{fpga} typically contain distributed \gls{sram} arrays, called \glspl{bram}. Several arrays can be combined by the vendor synthesis tool to create a larger array. With this in mind, the tool is able to flexibly define the dimensions of the buffers in \gls{rtl} generation to best suit the \gls{neural_net}. The system architecture contains one large \gls{bram} array for storing layer weights, and one for storing inter-layer results.

\subsection{Macro Generation}\label{sec:macro_gen}

A dedicated macro block is generated from a library of templates to process each type of layer (convolution, pooling, etc.) in the \gls{neural_net}. The \gls{rtl} templates are highly parameterized so that their dimensions can be defined when they are instantiated. Each macro block is controlled by the sequencer to loop over the input matrix using instructions written in microcode. The required macro blocks for a layer type and their design parameters are generated using the formulas outlined in the model analysis section.
...
The model analysis is a phase that bridges the software TensorFlow model (presented at this stage as a .tflite file) and the hardware architecture created for the model. In this phase, a script is used to extract the parameters required for designing the components for the final hardware. This extracts general information on the network being read without loss of generality:
\begin{enumerate}
...
Listing \ref{lst:sequencer_fsm} contains sections of abbreviated code from a sequencer state machine for 1x1 convolutions. The state machine contains the counting logic to iterate through the four loops outlined in \ref{sec:scheduler}. The address offset registers used in lines \ref{line:kernel_offset} and \ref{line:in_offset} stores the offset values to add to the base address in the microcode to obtain the correct set of data. Loop counters are compared to microcode base values (such as in line \ref{line:loop_compare}) to correctly loop over the input \glspl{tensor} and \glspl{kernel}.

\begin{figure}[htb]
\centering
\begin{lstlisting}[caption={Sequencer state machine}, label=lst:sequencer_fsm, language=Verilog, escapechar=|, numbers=left]
WR_OUT: begin
    // issuing writeout
    ...
    if (reg_kernel_count == NUM_KERNELS) begin
        next                    = IDLE;
    end else if (reg_tile_count == NUM_TILES-1) begin |\label{line:loop_compare}|
        ...
        tile_count_next    = 0;
        // kernel addr offset
        kernel_offset_next = reg_kernel_offset + NUM_INPUT_CHANNELS; |\label{line:kernel_offset}|
        // data addr offset
        out_ch_offset_next = reg_out_ch_offset + NUM_TILES; |\label{line:in_offset}|
        tile_offset_next   = 0;
    end else begin
